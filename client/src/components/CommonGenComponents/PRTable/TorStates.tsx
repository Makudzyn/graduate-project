...
import { fillZigZagMatrixWithArr, mergeSubArrays } from "../../../functions/generatorFunctions.ts";
...
const TorStates = ({ searchParams, degreeParamA, degreeParamB, conditionMatrix, factualPeriodA, factualPeriodB, basisMatrix}: TorStatesProps) => {
  ...
  const handleTorStatesCreation = () => {
    ...
    if (degreeA && degreeB && conditionMatrix) {
      ...
      const matrix = fillZigZagMatrixWithArr(completeMatrix, factualPeriodA, factualPeriodB);
