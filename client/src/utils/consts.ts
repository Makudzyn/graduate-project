//Routes/endpoints
export const MAIN_ROUTE = '/main';
export const ADMIN_PANEL_ROUTE = '/admin-panel';
export const LOGIN_ROUTE = '/login';
export const REGISTRATION_ROUTE = '/registration';
export const POLYNOMIALS_ROUTE = '/polynomials';
export const LINEAR_GEN_ROUTE = '/linear-generator';
export const MATRIX_GEN_ROUTE = '/matrix-generator';
export const SUM_AND_PRODUCT_GEN_ROUTE = '/sum-and-product-generator';
export const REGISTER_COMPARISON_ROUTE = '/register-comparison';
export const FROBENIUS_GEN_ROUTE = '/frobenius-generator';

//URL query params
export const PARAMS_DEGREE = 'degree';
export const PARAMS_DEGREE_A = 'degree_a';
export const PARAMS_DEGREE_B = 'degree_b';
export const PARAMS_POLYNOMIAL = 'polynomial';
export const PARAMS_POLYNOMIAL_A = 'polynomial_a';
export const PARAMS_POLYNOMIAL_B = 'polynomial_b';
export const PARAMS_CYCLIC_POLY_A = 'cyclic_a';
export const PARAMS_CYCLIC_POLY_B = 'cyclic_b';
export const PARAMS_OUTPUT_INDEX_I = 'index_i';
export const PARAMS_OUTPUT_INDEX_J = 'index_j';
export const PARAMS_MATRIX_RANK = 'matrix_rank';
export const PARAMS_USER_VALUE = 'value';
export const PARAMS_USER_VALUE_A = 'value_a';
export const PARAMS_USER_VALUE_B = 'value_b';
export const PARAMS_HAMMING_BLOCK = 'hamming_block';
export const PARAMS_DECOMPOSED_POLYNOMIAL = 'dec_poly';

//Polynomial types
export const POLYNOMIAL_TYPE_A = 'A';
export const POLYNOMIAL_TYPE_B = 'B';

export const REACT_APP_API_URL = 'http://localhost:5000/';
